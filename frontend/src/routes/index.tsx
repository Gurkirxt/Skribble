...
function MainPage() {
  ...
  const [username, setUsername] = useState(
    () => localStorage.getItem("skribble_username") || "",
  );
  const [usernameError, setUsernameError] = useState(false);

  const [traitIndices, setTraitIndices] = useState<Record<TraitName, number>>(
    () => {
      const stored = localStorage.getItem("skribble_trait_indices");
      if (stored) {
        try {
      ...
      return {
        top: Math.floor(Math.random() * TRAITS.top.length),
        eyes: Math.floor(Math.random() * TRAITS.eyes.length),
        mouth: Math.floor(Math.random() * TRAITS.mouth.length),
        clothing: Math.floor(Math.random() * TRAITS.clothing.length),
        clothesColor: Math.floor(Math.random() * TRAITS.clothesColor.length),
        skinColor: Math.floor(Math.random() * TRAITS.skinColor.length),
      };
    },
  );

  const [uuid] = useState(() => {
    let stored = localStorage.getItem("skribble_uuid");
    if (!stored) {
      stored = generateUUID();
      localStorage.setItem("skribble_uuid", stored);
    }
    return stored;
  });
  ...
  useEffect(() => {
    localStorage.setItem(
      "skribble_trait_indices",
      JSON.stringify(traitIndices),
    );
    localStorage.setItem("skribble_avatar", JSON.stringify(avatarConfig));
  }, [traitIndices, avatarConfig]);
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-background p-4 gap-8">
      ...
      <div className="flex flex-col md:flex-row gap-8 w-full justify-center items-center md:items-stretch">
        ...
        <div className="w-full flex-1 max-w-sm rounded-xl border bg-card p-6 shadow-sm flex flex-col items-center justify-center gap-6">
        ...
        <div className="w-full flex-1 max-w-sm rounded-xl border bg-card p-6 shadow-sm flex flex-col justify-center gap-4">
          <Button
            size="lg"
            ...
            <div className="flex flex-col items-center gap-4 pt-4 mt-2 border-t border-border">
              ...
              >
                <InputOTPGroup>
                  <InputOTPSlot
                    index={0}
                    className="h-12 w-12 text-base uppercase"
                  />
                  <InputOTPSlot
                    index={1}
                    className="h-12 w-12 text-base uppercase"
